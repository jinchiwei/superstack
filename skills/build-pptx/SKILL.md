...
**For the figure itself**: prefer the reusable archetype library at `skills/_shared/deck_figures.py` over hand-rolling matplotlib. The catalog is in [`figure_archetypes.md`](figure_archetypes.md) — includes 4-panel composite scorecards, region-dot strips, comparison-bar pairs, AIC sweeps, FDR survival grids, depth schematics, compartment partitions, and pipeline flows. All brand-locked, theme-aware, transparent-bg-by-default for slide composition. When you find yourself reaching for `plt.subplots()` to build a scorecard, check the catalog first — your archetype is probably already there.
