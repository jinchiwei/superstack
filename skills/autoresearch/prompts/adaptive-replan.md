...
8. **Target axis revisable mid-run.** The `state.target_axis` field can be updated if the framing shifts (e.g., started as a multi-pathogen sweep but pivoted to a single-pathogen deep-dive that now sweeps across train/eval splits — in which case `target_axis` would change from `"pathogen"` to `"split"`, or to `null` if there's no longer a meaningful target dimension). Include `target_axis` in the JSON output if you're changing it; omit if unchanged.
